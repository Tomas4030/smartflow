  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="tag urgent" style={{ alignSelf: "flex-start", marginBottom: 24 }}>
            <span className="dot" />
          ...
          <h1
            ...
            style={{
              ...
              maxWidth: 980, margin: 0, fontWeight: 500,
            }}
          >
            {t.hero.title_a}
            <br />
          ...
          <p style={{ fontSize: "clamp(16px, 1.4vw, 20px)", color: "var(--fg-dim)", maxWidth: 620, marginTop: 28, lineHeight: 1.55 }}>
            {t.hero.sub}
          </p>
          <div style={{ display: "flex", gap: 12, marginTop: 36, flexWrap: "wrap" }}>
            <a href="simulator.html" className="btn btn-primary">
        ...
        {step >= 1 && (
          <StepCaption step={step} lang={lang}/>
        )}
        ...
        <style>{`
          ...
          @keyframes sf-wheel-spin { to { transform: rotate(360deg) } }
          @keyframes sf-light-pulse { 0%,100% { opacity: 1 } 50% { opacity: .55 } }
          @keyframes sf-detect-ring { 0% { r: 18; opacity: .9 } 100% { r: 80; opacity: 0 } }
        `}</style>
      </div>
    </section>
  );
...
function StepCaption({ step, lang }) {
  const captions = lang === "en" ? [
    null,
    { eyebrow: "STEP 01 — THE PROBLEM",      title: "An ambulance hits red lights.", body: "Every red light costs precious seconds — sometimes lives." },
    { eyebrow: "STEP 02 — DETECTION",         title: "The AI camera sees it coming.", body: "Sirens, lights, vehicle silhouette — recognised in real time." },
    { eyebrow: "STEP 03 — ACTIVATION",        title: "Lights flip green ahead.",       body: "The intersection opens the ambulance's axis just before arrival." },
    { eyebrow: "STEP 04 — FREE PASSAGE",      title: "Time recovered. Lives saved.",   body: "5–7 minutes to the scene. 10–15 to the hospital." },
  ] : [
    null,
    { eyebrow: "PASSO 01 — O PROBLEMA",       title: "A ambulância encontra vermelhos.", body: "Cada semáforo custa segundos preciosos — às vezes vidas." },
    { eyebrow: "PASSO 02 — DETEÇÃO",           title: "A câmara IA vê-a chegar.",          body: "Sirene, luzes, silhueta — reconhecidas em tempo real." },
    { eyebrow: "PASSO 03 — ATIVAÇÃO",          title: "Os semáforos abrem à frente.",     body: "O cruzamento abre o eixo da ambulância antes da sua chegada." },
    { eyebrow: "PASSO 04 — PASSAGEM LIVRE",    title: "Tempo recuperado. Vidas salvas.",   body: "5–7 minutos até ao acidente. 10–15 até ao hospital." },
  ];
  ...
  return (
    ...
    >
      ...
      >
        <div className="mono" style={{ fontSize: 11, letterSpacing: "0.18em", color: "var(--accent)", marginBottom: 12 }}>
          {c.eyebrow}
        </div>
        <div className="display" style={{ fontSize: "clamp(28px, 3.4vw, 40px)", lineHeight: 1.1, fontWeight: 500 }}>
          {c.title}
        </div>
        <p style={{ marginTop: 14, color: "var(--fg-dim)", fontSize: 15, lineHeight: 1.55 }}>
          {c.body}
        </p>
      </div>
      <style>{`
        @keyframes sf-cap-in { from { opacity: 0; transform: translateY(8px) } to { opacity: 1; transform: none } }
      `}</style>
    </div>
  );
}
...
function ProgressDots({ step }) {
  return (
    ...
    >
      {[1, 2, 3, 4].map(i => (
        <div key={i} style={{
          display: "flex", alignItems: "center", gap: 10, opacity: i <= step ? 1 : 0.35,
          ...
          <span className="mono" style={{ fontSize: 10, color: "var(--fg-muted)", letterSpacing: "0.14em" }}>
            0{i}
          </span>
        </div>
      ))}
...
function RoadScene({ scrollP, detectionActive, step }) {
  const W = 1600, H = 700;
  const ROAD_TOP = 440, ROAD_BOTTOM = 560;
  const GROUND_Y = 580;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" preserveAspectRatio="xMidYMid slice" style={{ display: "block" }}>
      ...
      {Array.from({ length: 26 }).map((_, i) => {
        ...
        return <circle key={i} cx={x} cy={y} r={r} fill="oklch(0.85 0.03 285)" opacity={0.25 + (i % 3) * 0.1}/>;
      })}
      ...
      {Array.from({ length: 22 }).map((_, i) => (
        <rect key={i} x={i * 80 - 20} y={(ROAD_TOP + ROAD_BOTTOM)/2 - 2} width="48" height="3" fill="oklch(0.78 0.02 285)" opacity="0.55"/>
      ))}
...
function Skyline({ W, baseY }) {
  ...
  return (
    <g opacity="0.35">
      {pts.map((p, i) => (
        <rect key={i} x={p.x} y={baseY - p.h} width={p.w - 2} height={p.h} fill="oklch(0.18 0.02 285)"/>
      ))}
    </g>
  );
}
      ...
      {items.map((b, k) => (
        <g key={k}>
          ...
          {Array.from({ length: Math.floor(b.h / 14) }).map((_, r) => (
            <rect key={r}
              x={b.x + 6} y={y - b.h + 8 + r * 14}
              width={b.w - 12} height="3"
              fill={b.accent && r === 0 ? "var(--accent)" : "oklch(0.55 0.04 80)"}
...
function Zebra({ cx, top, bottom }) {
  const stripes = [];
  for (let i = 0; i < 7; i++) stripes.push(i);
  return (
    <g opacity="0.55">
      {stripes.map(i => (
        <rect key={i} x={cx - 30 + i * 9} y={top + 4} width="6" height={bottom - top - 8} fill="oklch(0.85 0.005 285)" opacity="0.6"/>
      ))}
    </g>
  );
}
...
function TrafficLight({ cx, groundY, green }) {
  ...
  return (
    <g>
      ...
      <text x={cx + 22} y={lampY + 6} fontSize="9" fontFamily="var(--font-mono)" fill={green ? "var(--green)" : "var(--red)"} letterSpacing="0.14em">
        {green ? "GO" : "STOP"}
      </text>
    </g>
  );
...
function Lamp({ cx, cy, color, on }) {
  return (
    <g>
      ...
      <circle cx={cx} cy={cy} r="6.5" fill={on ? color : "oklch(0.18 0.01 285)"} opacity={on ? 1 : 0.8}
        filter={on ? "url(#hero-glow)" : undefined}
        style={on ? { animation: "sf-light-pulse 1.6s ease-in-out infinite" } : undefined}/>
...
function Camera({ cx, groundY, active, ambX, ambY }) {
  ...
  return (
    <g>
      ...
      <text x={cx - 3} y={lampY - 9} textAnchor="middle" fontSize="8" fontFamily="var(--font-mono)" letterSpacing="0.16em" fill={active ? "var(--accent)" : "var(--fg-muted)"}>
        AI · CAM
      </text>

      {active && ambX < cx + 30 && ambX > cx - 700 && (
        <g opacity="0.6">
          <path d={`M ${cx - 38} ${lampY + 3}
                    L ${ambX - 30} ${ambY - 24}
                    L ${ambX - 30} ${ambY + 24} Z`}
            fill="var(--accent)" opacity="0.12"/>
          <line x1={cx - 38} y1={lampY + 3} x2={ambX - 30} y2={ambY - 24}
...
function Ambulance({ x, y, active }) {
  return (
    <g transform={`translate(${x}, ${y})`}>
      ...
      {active && (
        <ellipse cx="0" cy="50" rx="130" ry="14" fill="var(--accent)" opacity="0.18" filter="url(#hero-glow)"/>
      )}
