...
export default function AdminMunicipalities() {
  ...
  function openEdit(mun) {
    setEditing(mun);
    setName(mun.name);
    setDistrict(mun.district);
    setError("");
    setCreated(null);
    setShowForm(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!name.trim() || !district.trim()) {
      setError("Preencha nome e distrito.");
      return;
    }
    if (!editing && (!adminEmail.trim() || !adminPassword.trim())) {
      setError("Preencha email e password do administrador.");
      return;
    }
    setError("");
    try {
      const url = editing
        ? `/api/admin/municipalities/${editing.id}`
          ...
          >
            {created ? (
              <>
                ...
                >
                  ...
                  <div>
                    ...
                    <div
                      className="mono"
                      style={{ fontSize: 14, marginTop: 4, userSelect: "all" }}
                    >
                      {created.email}
                    </div>
                  </div>
                ...
                >
                  ⚠ Guarde estas credenciais — a password não pode ser
                  recuperada.
                </p>
                <button
                  onClick={closeForm}
              ...
              >
                ...
                >
                  <label className="mono" style={labelSty}>
                    Nome do Município
                  </label>
                  <input
                    value={name}
                    onChange={(e) => {
                      ...
                      if (!editing)
                        setAdminEmail(`admin@${toSlug(e.target.value)}.pt`);
                    }}
                    placeholder="Ex: Loulé"
                    style={inputSty}
                  />
                </div>
                <div
                    ...
                    >
                      ...
                      >
                        ...
                        <input
                          ...
                          placeholder="admin@municipio.pt"
                          style={inputSty}
                        />
                      </div>
                      <div
          ...
          <tbody>
            {municipalities.map((mun) => (
              ...
              >
                ...
                <td style={{ ...tdSty, fontSize: 15, fontWeight: 500 }}>
                  {mun.total}
                </td>
                <td style={tdSty}>
                  {mun.pending > 0 ? (
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: "var(--radius-sm)",
                        border:
                          "1px solid color-mix(in oklch, var(--red) 40%, var(--hairline))",
                        fontSize: 13,
                        color: "var(--red)",
                        cursor: "pointer",
                      }}
