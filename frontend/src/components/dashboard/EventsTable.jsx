...
function StatusBadge({ event }) {
  ...
  if (event._status === "alerta") {
    s = {
      ...
      c: "var(--red)",
      border: "color-mix(in oklch, var(--red) 35%, transparent)",
      label: "Alerta",
      dot: "var(--red)",
    };
  } else if (event._status === "analise") {
    s = {
      bg: "color-mix(in oklch, var(--amber) 14%, transparent)",
      c: "var(--amber)",
      border: "color-mix(in oklch, var(--amber) 35%, transparent)",
      label: "Em análise",
      dot: "var(--amber)",
    };
    ...
    s = {
      ...
      c: "var(--green)",
      border: "color-mix(in oklch, var(--green) 35%, transparent)",
      label: "Resolvido",
      dot: "var(--green)",
    };
  } else {
    ...
    >
      <span
        style={{
          ...
          borderRadius: "50%",
          background: s.dot,
          boxShadow: blink ? `0 0 8px ${s.dot}` : "none",
          animation: blink ? "db-blink 1s infinite" : "none",
        }}
      />
      {s.label}
    </span>
...
function TriggerBadge({ type }) {
  const map = {
    "Deteção Visual": {
      color: "var(--cyan)",
      bg: "color-mix(in oklch, var(--cyan) 10%, transparent)",
      icon: (
        <svg
          width="12"
          height="12"
    ...
    "Ciclo Restaurado": {
      color: "var(--green)",
      bg: "color-mix(in oklch, var(--green) 10%, transparent)",
      icon: (
        <svg
          width="12"
          height="12"
    ...
    "Falha de Sensor": {
      color: "var(--red)",
      bg: "color-mix(in oklch, var(--red) 10%, transparent)",
      icon: (
        <svg
          width="12"
          height="12"
        ...
        >
          ...
          <tbody>
            {visible.map((e, i) => {
              ...
              const displayType = e.resolvedAt
                ? types[1]
                : e.triggeredBy === "sos"
                  ? "SOS Cidadão"
                  : e.triggeredBy === "camera"
                    ? types[0]
                    : e._status === "alerta"
                      ? types[2]
                      : types[0];
                ...
                <tr
                  ...
                  style={{
                    background:
                      i % 2 === 0
                        ? "transparent"
                        : "color-mix(in oklch, var(--surface) 20%, transparent)",
                ...
                >
                  ...
                  >
                    <div style={{ color: "var(--fg)" }}>
                      {new Date(e.detectedAt).toLocaleTimeString("pt-PT", {
                        hour: "2-digit",
                        minute: "2-digit",
                        second: "2-digit",
                      })}
            ...
            {visible.length === 0 && (
              <tr>
                ...
                >
                  ...
                  >
                    Os eventos aparecem quando o sistema deteta uma emergência
                  </div>
                </td>
              </tr>
            )}
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
            ...
            {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
              <button
                ...
                style={{
                  ...
                  fontSize: 11,
                  cursor: "pointer",
                  borderColor: p === page ? "var(--accent)" : "var(--hairline)",
            ...
            <button
              ...
              style={{
                ...
                border: "1px solid var(--hairline)",
                background: "var(--surface)",
                color: page === pages ? "var(--fg-muted)" : "var(--fg)",
