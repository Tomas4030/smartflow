...
export function SectionHead({ eyebrow, title, sub, align = "left" }) {
  return (
    <div style={{ textAlign: align, marginBottom: 56, maxWidth: align === "center" ? 820 : 900, marginLeft: align === "center" ? "auto" : 0, marginRight: align === "center" ? "auto" : 0 }}>
      ...
      <h2 className="display" style={{ fontSize: "clamp(36px, 5vw, 64px)", margin: 0, fontWeight: 500 }}>{title}</h2>
      {sub && <p style={{ marginTop: 20, fontSize: 18, lineHeight: 1.55, color: "var(--fg-dim)", maxWidth: 720 }}>{sub}</p>}
    </div>
  );
}
  ...
  return (
    <section style={{ padding: "140px 0 80px" }}>
      <div className="container">
        ...
        <Reveal delay={80}>
          <h2 className="display" style={{ fontSize: "clamp(40px, 6.4vw, 88px)", lineHeight: 1.05, fontWeight: 500, margin: 0, maxWidth: 1100 }}>{t.intro.headline}</h2>
        </Reveal>
        <Reveal delay={160}>
          <p style={{ marginTop: 36, fontSize: 19, lineHeight: 1.6, color: "var(--fg-dim)", maxWidth: 760 }}>{t.intro.body}</p>
        </Reveal>
      </div>
    </section>
  );
            ...
            {t.solution.points.map((p, i) => (
              <Reveal key={i} delay={i * 90}>
                <div style={{ display: "flex", gap: 20 }}>
                  <div style={{ flexShrink: 0, width: 36, height: 36, borderRadius: 8, border: "1px solid var(--hairline)", display: "grid", placeItems: "center", fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--accent)", background: "color-mix(in oklch, var(--accent) 8%, transparent)" }}>0{i + 1}</div>
                  <div>
  ...
  return (
    <div style={{ position: "relative", aspectRatio: "1 / 1", border: "1px solid var(--hairline)", borderRadius: "var(--radius-lg)", background: "color-mix(in oklch, var(--surface) 50%, transparent)", overflow: "hidden" }}>
      ...
      <svg viewBox="0 0 400 400" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        ...
        <path d="M 110 130 Q 150 165 175 195" stroke="var(--accent)" strokeWidth="1.4" fill="none" markerEnd="url(#arrow)" strokeDasharray="4 4"><animate attributeName="stroke-dashoffset" from="0" to="-16" dur="1.2s" repeatCount="indefinite"/></path>
        <path d="M 225 195 Q 270 165 290 130" stroke="var(--accent)" strokeWidth="1.4" fill="none" markerEnd="url(#arrow)" strokeDasharray="4 4"><animate attributeName="stroke-dashoffset" from="0" to="-16" dur="1.2s" repeatCount="indefinite"/></path>
...
export function DiagramNode({ cx, cy, label, sub, emphasized }) {
  return (
    <g>
      <rect x={cx - 56} y={cy - 22} width="112" height="44" rx="6" fill={emphasized ? "color-mix(in oklch, var(--accent) 14%, var(--surface))" : "var(--surface)"} stroke={emphasized ? "var(--accent)" : "var(--hairline-2)"}/>
    ...
    <section ref={ref} style={{ padding: "120px 0", borderTop: "1px solid var(--hairline)" }}>
      <div className="container">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 28, marginTop: 40 }}>
          <RaceTrack label={t === SF_I18N.pt ? "SEM Smart Flow" : "WITHOUT Smart Flow"} time="00:18:42" color="var(--red)" progress={withoutSF}/>
          <RaceTrack label={t === SF_I18N.pt ? "COM Smart Flow" : "WITH Smart Flow"} time="00:11:28" color="var(--green)" progress={withSF} highlight/>
        </div>
      </div>
    </section>
  );
          ...
          {t.market.cards.map((c, i) => (
            <Reveal key={i} delay={i * 100}>
              <div style={{ padding: 32, border: "1px solid var(--hairline)", borderRadius: "var(--radius-lg)", background: "var(--surface)", height: "100%", display: "flex", flexDirection: "column", gap: 24, position: "relative", overflow: "hidden" }}>
                ...
                <div className="display" style={{ fontSize: "clamp(28px, 3.4vw, 40px)", fontWeight: 500, lineHeight: 1.1, position: "relative" }}>{c.v}</div>
            ...
            {t.roadmap.steps.map((s, i) => (
              <Reveal key={i} delay={i * 70}>
                <div style={{ display: "flex", gap: 28, alignItems: "flex-start" }}>
                  <div style={{ flexShrink: 0, width: 48, height: 48, borderRadius: "50%", border: "1px solid var(--accent)", background: "var(--bg)", display: "grid", placeItems: "center", color: "var(--accent)", fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.08em", boxShadow: "0 0 24px color-mix(in oklch, var(--accent) 35%, transparent)" }}>{s.y}</div>
    ...
    <section style={{ padding: "120px 0", borderTop: "1px solid var(--hairline)" }}>
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, border: "1px solid var(--hairline)", borderRadius: "var(--radius-lg)", overflow: "hidden" }} className="sf-compare-grid">
          <Reveal><CompareSide name={t.compare.a_name} pts={t.compare.a_pts} dim/></Reveal>
          <Reveal delay={120}><CompareSide name={t.compare.b_name} pts={t.compare.b_pts} highlight/></Reveal>
        </div>
...
export function CompareSide({ name, pts, dim, highlight }) {
  return (
    <div style={{ padding: 40, background: highlight ? "color-mix(in oklch, var(--accent) 8%, var(--surface))" : "var(--surface)", borderRight: "1px solid var(--hairline)", height: "100%", position: "relative" }}>
      <div className="display" style={{ fontSize: 28, fontWeight: 500, marginBottom: 4, color: highlight ? "var(--accent)" : "var(--fg)" }}>{name}</div>
      <div className="mono" style={{ fontSize: 11, letterSpacing: "0.14em", color: "var(--fg-muted)", marginBottom: 28 }}>{highlight ? "RECOMMENDED" : "ALTERNATIVE"}</div>
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 16 }}>
        {pts.map((p, i) => (
          <li key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start", color: dim ? "var(--fg-dim)" : "var(--fg)", fontSize: 15, lineHeight: 1.45 }}>
            <span style={{ flexShrink: 0, width: 18, height: 18, borderRadius: "50%", display: "grid", placeItems: "center", marginTop: 1, background: highlight ? "var(--accent)" : "transparent", border: highlight ? "0" : "1px solid var(--hairline-2)", color: "#fff", fontSize: 11 }}>{highlight ? "✓" : "·"}</span>
            <span>{p}</span>
          </li>
        ))}
      </ul>
    ...
    <section style={{ padding: "120px 0", borderTop: "1px solid var(--hairline)" }}>
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }} className="sf-pricing-grid">
          <Reveal><PricingCard lines={t.pricing.lines} total={t.pricing.total} title={t === SF_I18N.pt ? "Custo por cruzamento" : "Cost per intersection"}/></Reveal>
          <Reveal delay={120}><PricingCard lines={t.pricing.invest_lines} total={t === SF_I18N.pt ? "€55.000 total" : "€55,000 total"} title={t.pricing.invest_title} secondary/></Reveal>
        </div>
...
export function PricingCard({ title, lines, total, secondary }) {
  return (
    <div style={{ padding: 32, border: "1px solid var(--hairline)", borderRadius: "var(--radius-lg)", background: secondary ? "var(--surface)" : "color-mix(in oklch, var(--accent) 6%, var(--surface))", height: "100%", display: "flex", flexDirection: "column" }}>
      <div className="mono" style={{ fontSize: 11, letterSpacing: "0.14em", color: "var(--fg-muted)", marginBottom: 8 }}>{title}</div>
...
export function Avatar({ name, idx }) {
  ...
  const palette = [["oklch(0.50 0.22 300)", "oklch(0.30 0.15 300)"], ["oklch(0.55 0.16 200)", "oklch(0.30 0.12 200)"]];
  const [a, b] = palette[idx % palette.length];
  return (
    <div style={{ flexShrink: 0, width: 64, height: 64, borderRadius: 12, background: `linear-gradient(135deg, ${a}, ${b})`, display: "grid", placeItems: "center", color: "#fff", fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 22, letterSpacing: "-0.02em", border: "1px solid var(--hairline-2)" }}>{initials}</div>
  );
}
  ...
  return (
    <section style={{ padding: "120px 0", borderTop: "1px solid var(--hairline)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="container" style={{ position: "relative", textAlign: "center" }}>
        <Reveal><div className="eyebrow" style={{ marginBottom: 18 }}>{t.contact.eyebrow}</div></Reveal>
        <Reveal delay={80}><h2 className="display" style={{ fontSize: "clamp(36px, 5.5vw, 72px)", fontWeight: 500, margin: 0, maxWidth: 920, marginInline: "auto" }}>{t.contact.title}</h2></Reveal>
        <Reveal delay={160}><p style={{ marginTop: 24, fontSize: 18, color: "var(--fg-dim)" }}>{t.contact.sub}</p></Reveal>
        <Reveal delay={220}>
  ...
  return (
    <footer style={{ padding: "40px 0 60px", borderTop: "1px solid var(--hairline)" }}>
      <div className="container" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
        ...
        <div className="mono" style={{ fontSize: 12, color: "var(--fg-muted)" }}>{t.contact.email}</div>
      </div>
    </footer>
  );
}
